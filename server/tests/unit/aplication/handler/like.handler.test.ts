...
import { ClaimMother } from '../../../shared/mothers/ClaimMothers'
...
describe('unit - like handler tests', () => {
  ...
  test('should fail when visitor already liked', async () => {
    ...
    // act
    await expect(sut.execute(command)).rejects.toThrowError('Visitor already liked this claim.')
  })
  ...
  test('should fail when visitor pin does not match', async () => {
    ...
    // act
    await expect(sut.execute(command)).rejects.toThrowError('Invalid PIN')
  })
})
